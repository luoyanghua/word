...
/* 云存储  DataStorageWarning 存储空间不足警告 20180525 */
...
INSERT INTO SmsTemplate (TemplateCode, TemplaeName, Content, ThirdTemplateId, ServiceCenterId, InstitutionGuid) VALUES ('ExpertNISchedulingConfirm', N'远程会诊通知会诊专家新增已调度会诊任务（非交互式）', N'由{RequestCustomerName}提交的会诊申请（{PatientName}，{KindName}）已指派给您，请尽快处理。', NULL, NULL, NULL);
INSERT INTO SmsTemplatePara (TemplateCode, ParaCode, ParaName, SortNo, TemplateId )  SELECT  'ExpertNISchedulingConfirm', N'RequestCustomerName', N'申请医生', 1, MAX(ID) FROM SmsTemplate;
...
INSERT INTO SmsTemplate (TemplateCode, TemplaeName, Content, ThirdTemplateId, ServiceCenterId, InstitutionGuid) VALUES ('ExpertISchedulingConfirm', N'远程会诊通知会诊专家新增已调度会诊任务（交互式）', N'由{RequestCustomerName}提交的会诊申请（{PatientName}，{KindName}）已指派给您，于{ConsultDate}开始，请您做好准备，准时参加。', NULL, NULL, NULL);
INSERT INTO SmsTemplatePara (TemplateCode, ParaCode, ParaName, SortNo, TemplateId )  SELECT  'ExpertISchedulingConfirm', N'RequestCustomerName', N'申请医生', 1, MAX(ID) FROM SmsTemplate;
